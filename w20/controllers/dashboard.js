...
exports.getDashboard = async (req, res, next) => {
  ...
  try {
    ...
    const getCategories = await Category.fetchAll().then(([rows]) => {
      for (let p of rows) {
        p.date = moment(p.date).format("MMM D, YYYY");
      }
      categories = rows;
    ...
    const getCategoryCount = await Category.getCount().then(([rows]) => {
      categoryCount = rows[0].count;
    });

    const getUserCount = await User.getCount().then(([rows]) => {
